(() => {
    ...
    function createParserButton(){
        ...
        const selected=Lampa.Storage.get('parser_select')||'Jacred.xyz';
        btn.innerHTML=`<span>Парсер</span><div id="parser-current">${selected}</div>`;
        ...
        btn.addEventListener('click',(e)=>{
            ...
            PARSERS.forEach(p=>{
                ...
                item.className='selectbox-item selector'+(selected===p?' selected':'');
    ...
    function initPlugin(){
        ...
        // наблюдение за .torrent-filter
        const obs=new MutationObserver(()=>setTimeout(createParserButton,50));
        obs.observe(document.body,{childList:true,subtree:true});
    }

    function registerPlugin(){
        if(window.app && app.plugins && typeof app.plugins.add==='function'){
            app.plugins.add({id:plugin_id_menu,name:plugin_name_menu,version:'7.5',author:'maxi3219',description:'Меню + reload + парсер',init:initPlugin});
        } else initPlugin();
    }
