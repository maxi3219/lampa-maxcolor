(() => {
    ...
    // --- Добавление кнопки выбора парсеров ---
    function addParserButton() {
        ...
        parserButton.addEventListener('click', () => {
            if(typeof Lampa.SettingsApi !== 'undefined'){
                // Создаём меню выбора парсера динамически
                Lampa.SettingsApi.addParam({
                    ...
                    param: {
                        ...
                        values: {
                            ...
                            jac_red_ru: 'jac-red.ru'
                        },
                        default: 'no_parser'
                    },
                    field: {
                        name: '<div class="settings-folder" style="padding:0!important"><div style="font-size:1.0em">Выбор парсера</div></div>',
                        description: 'Нажмите для выбора парсера'
                    },
                    onChange(value){
                        Lampa.Storage.set('lme_url_two', value);
                        console.log('Выбран парсер:', value);
                    }
                });

                // Открываем меню выбора парсеров
                Lampa.SettingsApi.open('parser');
            } else {
                console.warn('Lampa.SettingsApi не доступен');
            }
        });
    ...
    function startObserver() {
        ...
        log('Observer started (v1.2, с выбором парсеров)');
    }
